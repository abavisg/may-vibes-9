...
export default function CourseCardsView({ params }: CourseCardsViewProps) {
  ...
  // For now, let's keep it but be aware it might need adjustment.
   useEffect(() => {
    ...
    if (!showResumeOptions && courseState.id && courseState.currentCardIndex !== initialLoadedIndex) {
       console.log(`CourseCardsView useEffect: Updating URL to startIndex ${courseState.currentCardIndex}`);
      const newUrl = `/course/${courseState.id}/cards?startIndex=${courseState.currentCardIndex}`;
      navigate(newUrl, { replace: true });
    }
  }, [courseState.currentCardIndex, courseState.id, navigate, showResumeOptions, selectedCourse?.currentCardIndex]);
